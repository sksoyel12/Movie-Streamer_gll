      ...
      Animated.timing(tagOpacity, { toValue: 1, duration: 500, useNativeDriver: true }).start(() => {
        setTimeout(() => {
          Animated.timing(splashOpacity, { toValue: 0, duration: 550, useNativeDriver: true }).start(() => {
            setPhase("profiles");
            Animated.parallel([
              Animated.timing(profOpacity, { toValue: 1, duration: 450, useNativeDriver: true }),
              Animated.spring(profY, { toValue: 0, damping: 18, stiffness: 120, useNativeDriver: true }),
      ...
      {phase !== "splash" && (
        <Animated.View style={[StyleSheet.absoluteFill, { opacity: profOpacity }]}>
          ...
          <Animated.View style={[pf.stageShell, { transform: [{ translateY: profY }] }]}>
            <LinearGradient
              colors={[
                "rgba(0,0,0,0)",       // transparent at the very top of curve
                "rgba(0,0,0,0.55)",    // soft dark mid
                "rgba(0,0,0,0.88)",    // strong dark
                "#000000",             // solid at bottom — profile content area
              ]}
              locations={[0, 0.22, 0.44, 0.72]}
              style={pf.stageGradient}
            >
              {/* Content constrained to real screen width inside the wide stage */}
              <View style={[pf.stageContent, { paddingBottom: Math.max(insets.bottom + 36, 52) }]}>

                {/* "Choose your profile" title */}
