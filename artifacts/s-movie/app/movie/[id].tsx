...
export default function MovieDetail() {
      ...
      <View style={styles.container}>
        ...
        >
          <View style={[styles.hero, { height: instantHeroH, backgroundColor: "#0d0d0d" }]}>
            {/* Dark gradient placeholder — NO portrait poster in hero */}
            <LinearGradient
              colors={["#1a1a2e", "#0d0d0d", "#000"]}
              locations={[0, 0.5, 1]}
  ...
  // 16:9 landscape ratio — matches Netflix mobile detail page hero exactly
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={[styles.hero, { height: heroHeight }]}>
          ...
          <LinearGradient
            colors={["transparent", "rgba(0,0,0,0.18)", "rgba(0,0,0,0.72)", "#000"]}
          ...
          <Pressable
            onPress={() => {
              ...
              router.push({
                pathname: "/player",
                params: { id: movie?.id ?? "", type: isTV ? "tv" : "movie", title_param: movie?.title ?? "" },
              });
            }}
        ...
        <View style={styles.body}>
          ...
          {(movie?.genres?.length ?? 0) > 0 && (
            <Text style={styles.genresLine} numberOfLines={1}>
              {(movie?.genres ?? []).join(" • ")}
            </Text>
          )}
