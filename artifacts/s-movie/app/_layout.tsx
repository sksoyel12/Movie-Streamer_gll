...
import { CURRENT_VERSION, LATEST_RELEASE, type ReleaseNote } from "@/data/releaseNotes";
import {
...
const WHATS_NEW_SEEN_KEY = "smovie_whats_new_version";
...
function WhatsNewModal({
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" statusBarTranslucent>
      <View style={wnStyles.backdrop}>
        <View style={wnStyles.sheet}>

          {/* Brand accent bar */}
          <View style={wnStyles.accentBar} />

          {/* Header */}
          <View style={wnStyles.header}>
            <View style={wnStyles.versionBadge}>
              <Text style={wnStyles.versionBadgeText}>v{release.version}</Text>
            </View>
            <Text style={wnStyles.headerTitle}>What's New</Text>
            {release.title ? (
              <Text style={wnStyles.headerSubtitle}>{release.title}</Text>
            ) : null}
            <Text style={wnStyles.headerDate}>{release.date}</Text>
          </View>

          <View style={wnStyles.sep} />

          {/* Highlights */}
          <ScrollView
            style={wnStyles.scroll}
            contentContainerStyle={wnStyles.scrollContent}
            showsVerticalScrollIndicator={false}
            bounces={false}
          >
            <View style={wnStyles.highlightList}>
              {release.highlights.map((item, i) => (
                <View key={i} style={wnStyles.highlightRow}>
                  <View style={wnStyles.bulletWrap}>
                    <MaterialCommunityIcons name="check" size={11} color="#e50914" />
                  </View>
                  <Text style={wnStyles.highlightText}>{item}</Text>
                </View>
              ))}
            </View>
          </ScrollView>
          ...
          {/* Footer CTA */}
          <View style={wnStyles.footer}>
            <Pressable
              style={({ pressed }) => [wnStyles.gotItBtn, pressed && { opacity: 0.88 }]}
              onPress={onDismiss}
            >
              <Text style={wnStyles.gotItText}>Continue to S-MOVIE</Text>
            </Pressable>
          </View>
...
export default function RootLayout() {
  ...
  const dismissWhatsNew = async () => {
    ...
    try {
      await AsyncStorage.setItem(WHATS_NEW_SEEN_KEY, CURRENT_VERSION);
    } catch {}
  };
